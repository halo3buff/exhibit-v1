      ...
      {images.map((img, i) => {
        ...
        return (
          <div key={img.id} style={{ position: 'absolute', top: slot.top, left: slot.left, width: slot.width, opacity: slot.opacity, animation: `fade-in 1s ease ${i * 0.15}s both`, pointerEvents: 'none' }}>
            <img src={hqUrl(img.imageUrl)} alt="" style={{ width: '100%', height: 'auto', display: 'block' }} onError={e => { e.target.style.opacity = '0'; }} />
          </div>
        );
      })}

      <div style={{ position: 'relative', zIndex: 10, background: '#f9f9f8', padding: '44px 48px 40px', boxShadow: '0 4px 32px rgba(0,0,0,0.12), 0 0 0 1px rgba(0,0,0,0.05)', width: 400, animation: 'fade-up 0.6s ease both' }}>
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          <div style={{ fontFamily: 'var(--font-sans)', fontSize: 22, fontWeight: 700, color: '#1a1a1a', lineHeight: 1.2, marginBottom: 2 }}>
            Create account
          </div>
...
function Field({ label, type, value, onChange }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      <label style={{ fontFamily: 'var(--font-mono)', fontSize: 8, letterSpacing: '0.22em', textTransform: 'uppercase', color: 'rgba(0,0,0,0.4)' }}>{label}</label>
      <input type={type} value={value} onChange={e => onChange(e.target.value)} required style={{ fontFamily: 'var(--font-sans)', fontSize: 14, fontWeight: 400, background: 'transparent', border: 'none', borderBottom: '1px solid rgba(0,0,0,0.15)', padding: '7px 0', color: '#1a1a1a', outline: 'none', width: '100%' }} />
    </div>
  );
}