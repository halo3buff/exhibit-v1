// src/pipeline/02-transform/02-transform-letterformarchive.ts
...
const SOURCE: SourceName = 'letterformarchive';

async function main() {
  console.log('╔═══════════════════════════════════════════════════════════╗');
  console.log('║  LETTERFORM ARCHIVE TRANSFORM                            ║');
  ...
  for (const filePath of files) {
    try {
      ...
      if (result.accepted && result.item) {
        ...
        if (accepted % 250 === 0) console.log(`   ✓ ${accepted} accepted...`);
      } else {
        rejected++;
      }
    } catch (e: any) {
