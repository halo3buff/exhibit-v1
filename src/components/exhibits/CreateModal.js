    ...
    <div onClick={onCancel} style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(232,229,222,0.94)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: 'var(--bg)', border: '1px solid var(--border-md)', padding: '52px 56px', width: 440, display: 'flex', flexDirection: 'column', gap: 18 }}>
        ...
        <input className="create-modal-title-input" value={title} onChange={e => setTitle(e.target.value)} placeholder="Title" autoFocus
          ...
          style={{ fontFamily: 'var(--font-sans)', fontSize: 28, fontWeight: 300, letterSpacing: '-0.02em', color: 'var(--fg)', background: 'transparent', border: 'none', outline: 'none', width: '100%', padding: '4px 0' }} />
        <input className="create-modal-desc-input" value={desc} onChange={e => setDesc(e.target.value)} placeholder="Description (optional)"
          onKeyDown={e => { if (e.key === 'Enter' && title.trim()) onSubmit(title, desc); if (e.key === 'Escape') onCancel(); }}
          style={{ fontFamily: 'var(--font-sans)', fontSize: 12, fontWeight: 400, color: 'var(--fg-muted)', background: 'transparent', border: 'none', outline: 'none', width: '100%', padding: '4px 0' }} />
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 4 }}>
          <button onClick={() => title.trim() && onSubmit(title, desc)} disabled={creating || !title.trim()}
            style={{
              ...
              color:         creating || !title.trim() ? 'var(--fg-faint)' : 'var(--bg)',
              background:    creating || !title.trim() ? 'var(--bg-card)' : 'var(--fg)',
              border:        'none',
              padding:       '11px 28px',
          ...
          <button onClick={onCancel} style={{ fontFamily: 'var(--font-mono)', fontSize: 8, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--fg-faint)', background: 'none', border: 'none', cursor: 'pointer' }}>Cancel</button>
        </div>
      </div>
    </div>
  );
